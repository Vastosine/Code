...
struct People {
    int l, r;
    bool operator<(const People &people) const { return r * l < people.r * people.l; }
};
...
int main() {
    ...
    cin >> king.l >> king.r;
    int m = 0;
    for (People &i : a) cin >> i.l >> i.r;
    for (int i = 1; i < n; i++) if (a[i].l * a[i].r > a[m].l * a[m].r) m = i;
    std::swap(a[m], a.back());
    std::sort(a.begin(), a.end() - 1);
    BigInt ans = 0, before = king.l;
    for (const People &i : a) {
        ans = std::max(ans, before / i.r);
        before *= i.l;
    }
    cout << ans;
}